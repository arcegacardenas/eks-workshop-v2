...
resource "null_resource" "change_config" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      # Skip creating directory if it already exists
      [ -d /eks-workshop/temp ] || mkdir -p /eks-workshop/temp
      ...
      aws eks update-addon --addon-name vpc-cni --cluster-name ${self.triggers.cluster_name} \
        --service-account-role-arn ${self.triggers.role_arn} \
        --configuration-values $NEW_CONFIG

      aws eks wait addon-active --cluster-name ${self.triggers.cluster_name} \
        --addon-name vpc-cni 

      # Run the remaining commands in parallel where possible
      echo "Detaching IAM policy and updating nodegroup config..."
      aws iam detach-role-policy --role-name ${self.triggers.role_name} \
        --policy-arn arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy

      aws eks update-nodegroup-config --cluster-name ${self.triggers.cluster_name} \
        --nodegroup-name ${self.triggers.node_group_name} \
        --scaling-config minSize=0,maxSize=6,desiredSize=1
