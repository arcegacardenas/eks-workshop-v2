  ...
  provisioner "local-exec" {
    command = <<-EOT
      aws eks create-nodegroup \
        ...
        --tags created-by=eks-workshop-v2,env=${data.aws_eks_cluster.cluster.id}

      echo "Waiting for nodegroup to leave CREATING state..."
      while true; do
        STATUS=$(aws eks describe-nodegroup --cluster-name ${data.aws_eks_cluster.cluster.id} --nodegroup-name new_nodegroup_3 --query 'nodegroup.status' --output text 2>/dev/null)
        if [ "$STATUS" != "CREATING" ] && [ -n "$STATUS" ]; then
